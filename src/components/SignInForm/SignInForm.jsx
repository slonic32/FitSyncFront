...
export default function SignInForm() {
    ...
    dispatch(login(data))
      ...
      .catch(error => {
        if (error === "Email is not verified") {
          toast.error(
            "Please verify your email. We will resend the verification link.",
            {
              duration: 4000,
            }
          );
  ...
  const onForgotPassword = () => {
    ...
    dispatch(requestPasswordReset(email))
      .unwrap()
      .then(() => {
        toast.success("Reset link sent to your email.", { duration: 4000 });
      })
      .catch(error => {
        toast.error(error || "Unable to send reset link. Try again later.", {
          duration: 4000,
        });
      });
  };
